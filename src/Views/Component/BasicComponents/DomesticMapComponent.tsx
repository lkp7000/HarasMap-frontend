...
const DomesticMapComponent: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const northEast = L.latLng(13.2, -75);
    ...
    const readMoreHandler = (data: any, event: any) => {
      console.log("Read More clicked with data:", data);
...
const cleanLongitude = separatedLongitude.replace(/[^\d.-]/g, '');
        ...
        //   zoom: map.getZoom,
        // };
        // const readMoreHandlerBind = readMoreHandler.bind(null, {
        //   lat: randomLat,
        //   lon: randomLon,
        //   zoom: map.getZoom(),
        // });
...
const cleanLongitude = separatedLongitude.replace(/[^\d.-]/g, '');
  ...
  const filterHandler = async (payload:any) => {
    try {
      const apiResponse = await filterVariousApi(payload);
        ...
        <div id="map" className="mt-28" style={{  width: "100%", height: "400px", zIndex: 1 }}>
          <div className="button-container flex flex-col lg:flex-row lg:justify-end">
            ...
            <button className="map-button button-2">
              <Link to ="/domesticviolence/chart">
                <div className="button-content-img">
...
export default DomesticMapComponent;
function selectRandomPoints(
  centerLat: number,
  centerLon: number,
  numPoints: number
